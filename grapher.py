...
def read_json_file(json_file_name):
    ...
    data=[]
    with open(json_file_name, 'r',encoding="utf-8") as file_handle:
        #data = [load(line) for line in file_handle]
...
if __name__ == '__main__':
    ...
    main_data = read_json_file('city.list.json')
    country_data = get_country_data(main_data, abbvs)
    country_cities = [city['name'] for city in country_data]
    city_id = get_city_id(country_data, country_cities)
    city_data = get_weather_data(city_id)
    generate_temperature_graph(city_data)
